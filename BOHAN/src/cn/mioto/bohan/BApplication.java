...
//	 public static String SOCKET_IP = "61.145.190.175";
	...
	/***************************************************/
	// UDP 变量
	private String BROADCAST_IP = "www.bohanserver.top";
	public static MulticastSocket multicastSocket;
	private DatagramPacket dpSend;
	private InetAddress serverAddress;
	private String message;// 发送的内容
	public int BROADCAST_PORT = 36868;// 目标端口
	public int BROADCAST_MYPORT = 34500;// 本机端口号
	/***************************************************/
	/*
	 * 用户绑定的所有设备，先保存在全局。
	 */
...
//		}
	...
	private void initUdp() throws IOException {
		multicastSocket = new MulticastSocket(BROADCAST_PORT);// 数据接收的目标端口
	 ...
	 * @param mess
	 */
	public void socketSend(final String mess) throws Exception{
		try {
			new Thread(new Runnable() {
						...
						try {
							if (!socket.isClosed()
								...
								if (!TextUtils.isEmpty(mess)) {
									...
									// BApplication.socket.setSoTimeout(10 *
									// 1000);
...
//	}
	...
	public String readString() throws Exception {
		if (null != socket) {
			if (in != null && !socket.isClosed() && !socket.isInputShutdown()) {
...
//				socket.close();
//			} catch (IOException e) {
//				LogUtilNIU.circle("BApplication Socket关闭异常");
//			}
//		}
//	}
}
