...
/**
 * 类说明：定时计量页面 (计算一定时间内用电量) 用socket收发（需要加上服务器数据和测试设备已经解决定时问题）
 */
public class MenuSetTimeSetActivity extends BaseCheckDataMenuActivity2
	...
	private Handler h = new Handler() {// 另一个hanlder
		public void handleMessage(Message msg) {
			if (msg.what == Constant.MSG_WHAT_DISMISS_DIALOG_OF_RESEND_TASK) {
				...
				// 显示正在加载
				if (udpKind == 0) {
					progressGettingDataShow(getString(R.string.last_measurement_data_query));
				} else if (udpKind == 1) {
					progressGettingDataShow(getString(R.string.in_the_metrological));
				}
			} else if (msg.what == Constant.MSG_WHAT_DISMISS_DIALOG_OF_RESEND_TASK_FAIL) {
				// 发送几次后，加载失败
				if (udpKind == 0) {
					progressGettingDataDismiss(getString(R.string.measurement_data_query_failed_last_time));
				} else if (udpKind == 1) {
					progressGettingDataDismiss(getString(R.string.failed_set_measurement_range));
				}
				...
				ToastUtils.longToast(MenuSetTimeSetActivity.this,getString(R.string.login_expired));
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ViewUtil.setToolbarTitle(getResources().getString(
				R.string.timing_setting));
		/*
		 * 查询设备资料，看有没有设置过定时计量，如果有， 返回结束时间，判断当前时间有没有到达结束时间. 如果没有到结束时间，显示进行中数据
		 * 如果到了结束时间。查询，显示 checkIfHasCount();
		 * 
		 * 如果没有设置，显示没有设置的本次计量页面
		 */
		receiver = new SetTimeBrocastReceiver();// 注册广播
		filter = new IntentFilter();
		filter.addAction(Constant.SOCKET_BROCAST_ONRECEIVED);
		filter.addAction(Constant.BROCAST_SET_TIME_JILIANG);
		filter.addAction(Constant.BROCAST_FAILE_MSG);
		filter.addAction(Constant.BROCAST_GET_TIME_JILIANG);
		filter.addAction(Constant.BROCAST_TIME_OUT);
		filter.addAction(Constant.BROCAST_SOCKET_FAIL);
		registerReceiver(receiver, filter);
		
		progressGettingDataShow(getString(R.string.query_information_measurement),true);
		checkNowData();
	}
	...
	protected void showContent(String content) {// 位数已经加10改好，但是实际需要设备测
		if (checkUDPMessage(content)) {
			if (isReqCodeEqual(content, "0007")) {// 查询定时计量的返回信息
				...
				String startTime = content.substring(24, 36);// 开始时间
																// YYMMDDHHMMSS
																// 年月日时分秒
				...
				String endTime = content.substring(36, 48);// 结束时间 YYMMDDHHMMSS
															// 年月日时分秒
				...
				if (ModbusCalUtil.calDisOfDate(d1, d2, 0).equals("small")) {
					...
					tvTotalUseTime.setText(ModbusCalUtil
							.calDisOfDate(d1, d2, 0));
				}
				/******** 上次结束时间****显示 ***************************************/
				tvLastEndDate.setText("20" + eyy + "-" + emo + "-" + edd);
				tvLastEndTime.setText(ehh + ":" + emm + ":" + ess);
				String durQuantity = content.substring(48, 56);// 期间用电量
																// 000000.00 kWh
				...
				if ((content.substring(24, 26).equals("00"))) {// 设置OK
					...
					tvThisTimeTitle.setText(getString(R.string.measured));
					// tvResultThis.setText(content);// 测试
					// TODO
					/*
					 * 把设置的开始时间和结束时间发送到服务器保存，下次进来是显示正在计量的状态
					 * 
					 * 改设备是否设置了定时计量 定时计量结束时间
					...
					ToastUtils.shortToast(MenuSetTimeSetActivity.this, getString(R.string.number_error));
				} else if (content.substring(24, 26).equals("02")) {// FLASH出错
					ToastUtils.shortToast(MenuSetTimeSetActivity.this,
		...
		// String startTime = content.substring(14,26);//开始时间 YYMMDDHHMMSS
		// 年月日时分秒
		// String yy = startTime.substring(0,2);
		// String mo = startTime.substring(2,4);
		// String dd = startTime.substring(4, 6);
		...
		// String disResTime =
		// disYY+"年"+disMM+"月"+disDD+"日"+disHH+"时"+disMIN+"分"+disSS+"秒";
		...
		// if((!content.substring(14,
		// 16).equals("01")&&!content.substring(14,16).equals("02"))||content.substring(14,
		// 16).equals("01")){//设置OK
	...
	protected void checkNowData() {
		...
		final String message = "E7" + deviceId + "00070000" + verCode + "0D";// 0001
																				// 查询用电参数
...
//				receiveThread.start();
	...
	public void onClick(View v) {
		...
		if (v == tvBeginToCount) {// 设置好开始和结束时间后，在这里发送定时计量指令
			if (!(startTime.equals("") || endTime.equals(""))) {// 开始时间不为空
				...
				String disNowAndStart = ModbusCalUtil.calDisOfDate(now, start,
						0);
				if (disNowAndStart.equals("small")) {
					ToastUtils.shortToast(MenuSetTimeSetActivity.this,
							getString(R.string.start_input_time_error));
				} else {
					try {
						String dis = ModbusCalUtil.calDisOfDate(start,
								sdf.parse("20" + endTime), 0);// 对比前后两个输入的时间的大小
						if (dis.equals("small")) {
							ToastUtils.shortToast(MenuSetTimeSetActivity.this,
									getString(R.string.end_input_time_error));
						} else {
			...
			/******** 弹出时间选择框****选择结束时间 **************************************/
			DateTimePicker picker2 = new DateTimePicker(this,
					DateTimePicker.HOUR_OF_DAY);
			picker2.setRange(2000, 2030);
			// 选择默认时间
			picker2.setSelectedItem(calendar.get(Calendar.YEAR),
					calendar.get(Calendar.MONTH) + 1,
					calendar.get(Calendar.DAY_OF_MONTH),
			...
			picker2.setOnDateTimePickListener(new DateTimePicker.OnYearMonthDayTimePickListener() {
				...
				public void onDateTimePicked(String year, String month,
					...
					endTime = year.substring(2) + month + day + hour + minute
							+ "30";
	...
	private void setCountTime() {// 设置定时计量
		udpKind = 1;
		String verCode = ModbusCalUtil.verNumber(deviceId + "0006000C"
				+ startTime + endTime);
		final String message = "E7" + deviceId + "0006000C" + startTime
				+ endTime + verCode + "0D";// 0001 查询用电参数
...
//				receiveThread.start();
	...
	//
	// String
	// distance=ModbusCalUtil.calDisOfDate(dateBegin,dateEnd);//计算两个date的时间差
	...
	public void onPause() {
		super.onPause();
		MobclickAgent.onPageEnd("定时计量"); // （仅有Activity的应用中SDK自动调用，不需要单独写）保证
	...
	public class SetTimeBrocastReceiver extends BroadcastReceiver {

		@Override
		public void onReceive(Context context, Intent intent) {
			String action = intent.getAction();
			 if(action.equals(Constant.BROCAST_GET_TIME_JILIANG)){
				 progressDismiss("");
				//开关操作
				...
				String message = intent
						.getStringExtra("message");
				ToastUtils.longToast(MenuSetTimeSetActivity.this, message);
			}else if(action.equals(Constant.BROCAST_SET_TIME_JILIANG)){
				progressGettingDataDismissNoToast();
				String message = intent
						.getStringExtra("message");
				showContent(message);
			}else if(action.equals(Constant.BROCAST_SOCKET_FAIL)){
				ToastUtils.longToast(MenuSetTimeSetActivity.this, getString(R.string.socket_disconnected));
			} else if(action.equals(Constant.BROCAST_TIME_OUT)){
				ToastUtils.longToast(MenuSetTimeSetActivity.this, getString(R.string.exception_sockettimeout));
			}
		}
	}
}
