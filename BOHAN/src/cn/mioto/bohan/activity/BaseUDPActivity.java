 ...
 * 
 * 作者： jiemai liangminhua 创建时间：2016年6月23日 下午6:08:25
 */
public class BaseUDPActivity extends SteedAppCompatActivity {
	...
	private int BROCAST_TIME_OUT_UDP = 50;
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					if(!gettedDataOK){
						gettedDataOK = true;
//						Intent intent = new Intent(Constant.BROCAST_TIME_OUT);
//						sendBroadcast(intent);
//						handler.sendEmptyMessage(BROCAST_TIME_OUT_UDP);
					}
...
//						handler.sendEmptyMessage(BROCAST_TIME_OUT_UDP);
	...
	protected Boolean checkUDPMessage(String content) {
		...
		String statCode = content.substring(14, 16);// 状态码
		LogUtilNIU.value("返回的状态码---" + statCode + "--id"
				+ content.substring(2, 14) + "deviceId--" + deviceId);
		String id = content.substring(2, 14);// ID
		if (id.equals(deviceId)) {// 判断接收到的信息的ID
	...
	private Handler handler = new Handler() {// 另一个hanlder
		public void handleMessage(Message msg) {
			if(msg.what == BROCAST_TIME_OUT_UDP){
				Log.d("exception_sockettimeout--------->", "exception_sockettimeoutexception_sockettimeout");
				ToastUtils.shortToast(BaseUDPActivity.this, getResources().getString(R.string.exception_sockettimeout));
			}
		}
	};
}
