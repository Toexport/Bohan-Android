...
public class MenuCountDownActivity extends BaseCheckDataMenuActivity2 implements
	...
	public Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			if (msg.what == UPDATE_COUNTTIME) {
				if (!((String) msg.obj).trim().equals("00 : 00 : 00")) {// 未结束时
					...
					handler.postDelayed(new Runnable() {
						...
						public void run() {
							...
							llNotSet.setVisibility(View.GONE);// 出现没有倒计时任务的提示
				...
				// 显示正在加载
				if (udpKind == 0) {
					progressGettingDataShow(getString(R.string.Countdown_to_update_information));
				} else if (udpKind == 1) {
					progressGettingDataShow(getString(R.string.set_countdown_on));
				} else if (udpKind == 2) {
					progressGettingDataShow(getString(R.string.set_countdown_off));
				}
			} else if (msg.what == Constant.MSG_WHAT_DISMISS_DIALOG_OF_RESEND_TASK_FAIL) {
				// 发送几次后，加载失败
				if (udpKind == 0) {
					progressGettingDataDismiss(getString(R.string.countdown_information_update_failure));
				} else if (udpKind == 1) {
					progressGettingDataDismiss(getString(R.string.countdown_setting_on_failure));
				} else if (udpKind == 2) {
					progressGettingDataDismiss(getString(R.string.countdown_setting_off_failure));
				} else if (udpKind == 3) {
					progressGettingDataDismiss(getString(R.string.Information_update_failed));
				}
				...
				ToastUtils.longToast(MenuCountDownActivity.this,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		receiver = new MenuCountBrocastReceiver();// 注册广播
		filter = new IntentFilter();
		filter.addAction(Constant.SOCKET_BROCAST_ONRECEIVED);
		filter.addAction(Constant.BROCAST_COUNT_DOWN);
		filter.addAction(Constant.BROCAST_FAILE_MSG);
		filter.addAction(Constant.BROCAST_NOW_DATA_MSG);
		filter.addAction(Constant.BROCAST_COUNT_DOWN_OPEN);
		filter.addAction(Constant.BROCAST_COUNT_DOWN_CLOSE);
	...
	private void checkLastCountDown() {
		...
		final String msg = "E7" + deviceId + "00120000" + verCode + "0D";// 0001
		// 查询用电参数
...
//				receiveThread.start();
	...
	public void onClick(View v) {
		if (v == llChooseTime1) {
			...
			picker.setLabel(getString(R.string.hour2), getString(R.string.min));
			...
			// 选择按钮后，按钮改变字体颜色为红色
			tvStatus.setText(getString(R.string.device_on));
			...
			/******* 关按钮 ********************************************/
			tvOnSelected = false;
			tvStatus.setText(getString(R.string.device_off));
			llStartGray.setVisibility(View.GONE);// 灰色按钮消失
			llStartBlue.setVisibility(View.VISIBLE);// 蓝色按钮出现
	...
	private void showContent(String content) {
		if (checkUDPMessage(content)) {
			if (isReqCodeEqual(content, "000B")) {// 倒计时开设置成功
				...
				progressGettingDataDismiss(getString(R.string.countdown_setting_on_success));
				...
				if (content.substring(24, 26).equals("00")) {// 设置OK
					...
					ToastUtils.shortToast(MenuCountDownActivity.this, getString(R.string.number_error));
				} else if (content.substring(24, 26).equals("02")) {// FLASH出错
					ToastUtils
							.shortToast(MenuCountDownActivity.this, getString(R.string.flash_error));
				}
			} else if (isReqCodeEqual(content, "000A")) {// 倒计时关设置成功
				// 把总时间保存在sp
				BApplication.instance.setResendTaskShowBreak(true);// 停止重发机制
				progressGettingDataDismiss(getString(R.string.countdown_setting_off_success));
				Editor e = BApplication.instance.getSp().edit();
				e.putString(deviceId + Constant.DEVICE_COUNT_DOWN_DURACTION,
						countDownTime + "00");// 手动加上秒
				e.commit();
...
//				} else 
					...
					if (content.substring(24, 26).equals("01")) {// 已设置倒计时开
...
//					llBottomSpace.setVisibility(View.GONE);
	...
	private void checkNowDeviceTime() {
		...
		if (!((BaseUDPActivity2) this).isFinishing()) {
			progressGettingDataShow(getString(R.string.state_being_updated));
		}
		udpKind = 3;
...
//				receiveThread.start();
	...
	private class ReceiveThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				Log.i("Receive", "socket receive:" + result);
				if (result != null) {
	...
	public class MenuCountBrocastReceiver extends BroadcastReceiver {

		@Override
		public void onReceive(Context context, Intent intent) {
			String action = intent.getAction();
			 if(action.equals(Constant.BROCAST_COUNT_DOWN)){
				 progressDismiss("");
				//开关操作
				...
				String message = intent
						.getStringExtra("message");
				ToastUtils.longToast(MenuCountDownActivity.this, message);
				...
				String message = intent
						.getStringExtra("message");
				showContent(message);
			}else  if(action.equals(Constant.BROCAST_COUNT_DOWN_OPEN)){
				 progressDismiss("");
				//开关操作
				String message = intent
						.getStringExtra("message");
				showContent(message);
			}else  if(action.equals(Constant.BROCAST_COUNT_DOWN_CLOSE)){
				 progressDismiss("");
				//开关操作
				String message = intent
						.getStringExtra("message");
				...
				ToastUtils.longToast(MenuCountDownActivity.this, getString(R.string.socket_disconnected));
			} else if(action.equals(Constant.BROCAST_TIME_OUT)){
				ToastUtils.longToast(MenuCountDownActivity.this, getString(R.string.exception_sockettimeout));
			}
		}
	}
}
