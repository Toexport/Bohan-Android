 ...
 * @author XiongChuanLiang<br/>(xcl_168@aliyun.com)
 */
public class LineChart02ViewHour extends BaseChartView implements Runnable{
	
	private String TAG = "LineChart02View";
	private LineChart chart = new LineChart();
	
	//标签集合（只是用于初始化，和用电数据无关的）下面这三个变量不用管
	private LinkedList<String> labels = new LinkedList<String>();
	private LinkedList<LineData> chartData = new LinkedList<LineData>();
	private List<CustomLineData> mCustomLineDataset = new LinkedList<CustomLineData>();
	
	
	// 以下几个变量是绘制这个项目的数据真正用到的
	//标签集合，x轴的标签，也就是时间
	...
	public LineChart02ViewHour(Context context) {
		super(context);
		// TODO Auto-generated constructor stub
//		initView();
	}
	
	public LineChart02ViewHour(Context context, AttributeSet attrs){   
        super(context, attrs);   
//        initView();
	 }
	 
	 public LineChart02ViewHour(Context context, AttributeSet attrs, int defStyle) {
...
//			initView();
	 ...
	 /***************************************************/
		...
		public void setBarColorAndData(List<Double> datas,Double max){
			...
			AreaData ratesLine = new AreaData("用电功率",rateDatas,
					getResources().getColor(R.color.chartColor2),getResources().getColor(R.color.areaColor));
			ratesLine.getDotLabelPaint().setTextSize(Constant.CHART_DATA_TEXT_SIZE);
			//设置线上每点对应标签的颜色
			ratesLine.getDotLabelPaint().setColor(Color.BLACK);
			ratesLine.setDotStyle(XEnum.DotStyle.DOT);	//点空心
			ratesLine.getDotPaint().setColor(getResources().getColor(R.color.red));
			//设置点标签
