...
* 创建时间：2016年7月21日 下午3:40:33 
* 
*/
public class SwitchViewLongOnline extends View{
    private int openColor = 0xff4ada60;//在现和离线时开的颜色
    private int closeColor = 0xffffffff;//在线时，关的颜色0xfff40a0a//  0xffffffff离线时关的颜色

	public void setCloseColorDark() {//把关闭状态颜色设置成灰色
		this.closeColor = 0xffffffff;
	}

	public void setCloseColorRed() {//把关闭状态颜色设置为红色
		this.closeColor = 0xfff40a0a;
	}
	 ...
	 * 关闭状态，不在线
	 */
	public static final int STATE_SWITCH_OFF_ONLINE = 5;
	/**
	 * current state
	 */
	...
	public SwitchViewLongOnline(Context context) {
		this(context, null);
	}

	public SwitchViewLongOnline(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int heightSize = (int) (widthSize * 0.48f);
	...
	@Override
	protected void onDraw(Canvas canvas) {//复写方法
		super.onDraw(canvas);
		paint.setAntiAlias(true);
		final boolean isOn = (state == STATE_SWITCH_ON || state == STATE_SWITCH_ON2);
		// draw background
		paint.setStyle(Style.FILL);
		paint.setColor(isOn ? 0xff4ada60 : 0xffe3e3e3);//打开时背景颜色 绿色  灰色
		...
		paint.setColor(closeColor);//关闭时 *****背景颜色  红色
		canvas.drawPath(sPath, paint);
		canvas.restore();
		// draw center bar
		canvas.save();
		...
		paint.setStyle(Style.FILL);
		paint.setColor(0xffffffff);//小圆
		canvas.drawPath(bPath, paint);

		paint.setStyle(Style.STROKE);
		paint.setStrokeWidth(bStrokeWidth * 0.5f);

		paint.setColor(isOn ? 0xff4ada60 : 0xffbfbfbf);//圆圈外围颜色
		canvas.drawPath(bPath, paint);
