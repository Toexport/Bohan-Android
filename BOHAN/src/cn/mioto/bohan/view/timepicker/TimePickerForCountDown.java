...
/**
 * 本小姐改写的时间选择器
 * 用于倒计时页面选择时间 xx小时xx分钟 小时最多是72小时
 *
 * @author jiemai liangminhua 
 */
public class TimePickerForCountDown extends WheelPicker {
    /**
     * 24小时
     */
    public static final int HOUR_CUSTOM = 0;
    /**
     * 12小时
     */
    ...
    private int mode;
    private String hourLabel = "时", minuteLabel = "钟";
     ...
     */
    @IntDef(flag = false, value = {HOUR_CUSTOM, HOUR})
     ...
     */
    public TimePickerForCountDown(Activity activity) {
        this(activity, HOUR_CUSTOM);
    }
     ...
     * @see #HOUR
     */
    public TimePickerForCountDown(Activity activity, @Mode int mode) {
        super(activity);
        this.mode = mode;
    ...
    public void setSelectedItem(int hour, int minute) {
        //
    	selectedHour = ModbusCalUtil.add0fillLength( String.valueOf(hour), 2);
        selectedMinute =  ModbusCalUtil.add0fillLength( String.valueOf(minute), 2);
    }

    /**
     * Sets on time pick listener.
     *
     * @param listener the listener  构造监听，带回调接口
     */
