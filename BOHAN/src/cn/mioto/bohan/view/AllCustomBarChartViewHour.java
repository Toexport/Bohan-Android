 ...
 * 
 */
public class AllCustomBarChartViewHour extends BaseChartView implements Runnable{

	private String TAG = "AllCustomBarChartViewHour";
	...
	public AllCustomBarChartViewHour(Context context) {
		super(context);
//		initView();
	}

	public AllCustomBarChartViewHour(Context context, AttributeSet attrs){   
		super(context, attrs);   
//		initView();
	}

	public AllCustomBarChartViewHour(Context context, AttributeSet attrs, int defStyle) {
...
//		initView();
	 ...
	 * @throws
	 */
	public void setBarColorAndData(List<String> color,List<Double> datas,Double max,List<SingleDeviceMonth> list){
		//初始化柱子颜色
		this.list = list;
		dataColorA.clear();
	...
	private void chartRender(Double max)
	{
		try {
			...
			chart.getPlotArea().extWidth(Float.parseFloat(max+""));
			...
			//如果没有采集到数据不显示数值，如果采集到数据，并且数据为0，显示数值为0
//			for (int i = 0; i < list.size(); i++) {
//				SingleDeviceMonth data = list.get(i);
//				if(data.getPowerIsNull().equals("1")){
