...
/** 
 * 类说明：单个设备月数据
 * 内嵌3个fragment 分别放每10天数据 共30天
 */
public class SingleDeviceMonthDataFragment2 extends BaseSingleDeviceDataFragment implements OnClickListener {
	private View view;
...
//		initX();
	...
	private void checkNowData() {
		...
		final String msg1="3A"+deviceId+"000E0000"+verCode +"0D";//0010 查询上24小时的用电量
	...
	protected void checkRate(){
		...
		final String msg2="3A"+deviceId+"000F0000"+verCode2+"0D";//0011 询上24小时的用电平均功率
	...
	private void initDay(String cyear,String cmonth,String ctoday) {
		...
		this.today=ctoday;
		LogUtilNIU.value("现在要初始化头顶的日期显示，传过来的年月日参数，"+"年"+this.year+"月"+this.thismonth+"日"+this.today);
		/*
		 * 月数据要考虑的情况
		 * 
		 * 当前月为1月
	...
	private void viewPagerAddContent() {
		view1=(LinearLayout) inflater.inflate(R.layout.view_single_device_month_data, null);
		view2=(LinearLayout) inflater.inflate(R.layout.view_single_device_month_data2, null);
		view3=(LinearLayout) inflater.inflate(R.layout.view_single_device_month_data3, null);
		view4=(LinearLayout) inflater.inflate(R.layout.view_single_device_month_data4, null);
		view5=(LinearLayout) inflater.inflate(R.layout.view_single_device_month_data5, null);
		views.add(view1);
		views.add(view2);
		views.add(view3);
		views.add(view4);
	...
	public class UDPBrocastReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(action.equals(Constant.SOCKET_BROCAST_ONRECEIVED)){
				...
				if(isReqCodeEqual(message, "000E")||isReqCodeEqual(message, "000F")){
					showContent(message);}
			}
		}
	}
	...
	public void showContent(String contentJson) {
		...
		if(checkUDPMessage(contentJson)){
			if(isReqCodeEqual(contentJson, "000E")){//单个设备月电量
				...
				LogUtilNIU.value("月数据的---年月日-------"+contentJson.substring(264, 270));
				//年月日 160923
	...
	private void initBarDatas(List<Double> powerDatas) {
		...
		bar11.setBarColorAndData(colors.subList(0, 6),powerDatas.subList(0, 6),max);
		bar21.setBarColorAndData(colors.subList(7, 12),powerDatas.subList(7, 12),max);
		bar31.setBarColorAndData(colors.subList(13, 18),powerDatas.subList(13, 18),max);
		bar41.setBarColorAndData(colors.subList(19, 24),powerDatas.subList(19, 24),max);
		bar51.setBarColorAndData(colors.subList(25, 30),powerDatas.subList(25, 30),max);
	}
	/***************************************************/
	/*
	 * 设备的用电功率的集合，是最近24小时的
	...
	protected void checkNowDataFromService(String smessage) {
		...
		new Enterface("sendToDevice.act").addParam("deviceid", deviceId).addParam("String content", smessage).doRequest(new JsonClientHandler2() {
			@Override
			...
			picker.setOnDatePickListener(new DatePicker.OnYearMonthPickListener() {
				...
				public void onDatePicked(String year, String month) {
					...
					if(isFuture(Integer.valueOf(year+month))){
						...
						checkDataFromeService(deviceId,"year_month",year+month);
