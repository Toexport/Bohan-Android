package cn.mioto.bohan.view.pop;
...
public class DropDownMenu extends LinearLayout{
    ...
    //菜单 上的文字
    private List<EditText> mEtMenuTitles=new ArrayList<>();
    
//    public Boolean isEditTextChange(){
//    	//判断输入框的内容有没有改变
//    	/*
//    	 * 思路，如果内容有变，getText前后数据不一样，也是就是数，个deFault的不一样
//    	 */
//    	String nowContent=mEtMenuTitles.get(0).getText().toString();
//    	String oldContent=mDefaultMenuTitle[0];
//    	if(nowContent.endsWith(oldContent)){
//    		return false;//如果现在的值和初始设置的值一样，则没有改变过
//    	}else{
//    		return true;
//    	}
//    }
    ...
    /**
     * @Title: getEditTextContent 
     * @Description:重写的方法，可以得到当前输入框的内容
     * @return String    
     * @throws
     */
    public String getEditTextContent() {
		return mEtMenuTitles.get(0).getText().toString();//获得editText内容
	}
    /**
     * @Title: getEditTextContent 
     * @Description:重写的方法，可以设置输入框的内容
     * @return String    
     * @throws
     */
	public void setEditTextContent(String title) {
		mEtMenuTitles.get(0).setText(title);
	}

	//菜单 的背景布局
    private List<LinearLayout> mRlMenuBacks=new ArrayList<>();
    //菜单 的箭头
        ...
        if (mDrawable) {
            ...
            mMenuList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    ...
                    mEtMenuTitles.get(mColumnSelected).setText(mMenuItems.get(mColumnSelected)[mRowSelected]);
            ...
            mPopupWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
                ...
                public void onDismiss() {
                    for (int i = 0; i < mMenuCount; i++) {
                        ...
                        mEtMenuTitles.get(i).setTextColor(mMenuTitleTextColor);
                    }
                }
            });
            ...
            for (int i = 0; i < mMenuCount; i++) {
                ...
                RelativeLayout.LayoutParams parms = new RelativeLayout.LayoutParams(width / mMenuCount, LayoutParams.MATCH_PARENT);
                v.setLayoutParams(parms);
                EditText et = (EditText) v.findViewById(R.id.et_menu_title);
                et.setTextColor(mMenuTitleTextColor);
                et.setTextSize(mMenuTitleTextSize);
                if (mDefaultMenuTitle==null||mDefaultMenuTitle.length==0){
                	et.setText(mMenuItems.get(i)[0]);//设置默认的显示
                }else{
                	et.setText(mDefaultMenuTitle[i]);
                }
                this.addView(v, i);
                mEtMenuTitles.add(et);

                LinearLayout rl = (LinearLayout) v.findViewById(R.id.rl_menu_head);
                ...
                v.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        mEtMenuTitles.get(index).setTextColor(mMenuPressedTitleTextColor);
