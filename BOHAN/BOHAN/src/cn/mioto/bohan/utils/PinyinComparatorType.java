...
/** 
 * 类说明：按类型比较的拼音的比较器,用于在线列表
 * 作者：  jiemai liangminhua 
 * 创建时间：2016年7月1日 下午2:55:31 
 */
public class PinyinComparatorType implements Comparator<SingleDevice> {
	...
	public int compare(SingleDevice o1, SingleDevice o2) {
		...
		if(o1.getIsOnline()==true&&o2.getIsOnline()==false){//先看是否在线，在线的为1
			...
			if(o1.getDeviceTypePinyin().compareTo(o2.getDeviceTypePinyin())>0){
				return 1;
			}else if(o1.getDeviceTypePinyin().compareTo(o2.getDeviceTypePinyin())<0){
				return -1;
			}else {
				return 0;
			}
