			...
			while(!countingToDismiss){
				...
				if(!countingToDismiss){
					if(!gettingDataOk){
						p.dismiss();
						gettingDataOk = true;
//						ToastUtils.shortToast(BaseUDPActivityNoCurrent.this, getResources().getString(R.string.loading_data_fail));
						LogUtilNIU.value("因为线程关系，这里用handler的timeout判断转圈消失了！！！！！！！！");
