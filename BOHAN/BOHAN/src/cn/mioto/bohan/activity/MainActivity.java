 ...
 */
public class MainActivity extends SteedAppCompatActivity implements
		OnClickListener {
	...
	private MainOnlineFragment listFragment;
	private MainListBangFragment listDeviceBangFragment;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int titleString;
		// 设置toolbar标题和toolbar;
		intTitle = R.string.device_list;// 最开始的时候显示为设备列表
		ViewUtil.initToolbar(this, R.id.toolbar, R.id.toolbar_title, intTitle,
				false, true, R.id.menu_tv, R.string.update);
		toolbarMenu = (TextView) findViewById(R.id.menu_tv);
		toolbarMenu.setVisibility(View.GONE);
		toolbarMenu.setOnClickListener(this);
		menu_front = (TextView) findViewById(R.id.menu_front);
		// toolbar上的刷新按钮，这里是要另外写监听的
		btnList.add(radio0);
		btnList.add(radio1);
		btnList.add(radio2);
		btnList.add(radio3);
		listFragment = new MainOnlineFragment();
		listDeviceBangFragment = new MainListBangFragment();
		...
		transaction.add(R.id.llContainer, listFragment);
		transaction.commit();
			...
			public void onInterfaceSuccess(String message, String contentJson) {
				...
				try {
					...
					for (int i = 0; i < arry.length(); i++) {
						sb.append(arry.getJSONObject(i).getString("name") + ",");
					}
					String locations = sb.toString();
					String key = Constant.SP_KEY_LOCATIONS;
					BApplication.instance.getEditor().putString(key, locations);
					BApplication.instance.getEditor().commit();
					LogUtilNIU.value("保存了到sp的品牌信息是"
							+ BApplication.instance.getSp().getString(
	...
	private void getLocationInfo() {
		new Enterface("getPositions.act").doRequest(new JsonClientHandler2() {
			@Override
			public void onInterfaceSuccess(String message, String contentJson) {
				LogUtilNIU.e("预先保存 返回 locations的content--->" + contentJson);
				try {
					JSONArray arry = new JSONArray(contentJson);
					StringBuffer sb = new StringBuffer();
					for (int i = 0; i < arry.length(); i++) {
	...
	public void onClick(View v) {
		if (v == ivScan) {
			...
			if (listDeviceBangFragment.howManyIsSelected().size() < 1) {// 没有选到设备
				ToastUtils.shortToast(MainActivity.this, "请选中需要分享的设备");
			} else {
		...
		if (v == radio0 || v == radio1 || v == radio2 || v == radio3) {
			if (clickIndex != currentFragmentIndex) {
				FragmentTransaction transaction = getSupportFragmentManager()
						.beginTransaction();
				Fragment showFragment = fragments[clickIndex];
				if (clickIndex == 1) {// 如果已绑设备被点击
					listDeviceBangFragment.update();
				}
			...
			LogUtilNIU.value("clickIndex---" + clickIndex);
			toolbarTitle = (TextView) findViewById(R.id.toolbar_title);
			toolbarTitle.setText(clickIndex == 0 ? getResources().getString(
					R.string.device_list) : (clickIndex == 1 ? getResources()
					.getString(R.string.manage_device)
					: clickIndex == 2 ? getResources().getString(
							R.string.all_info) : getResources().getString(
							R.string.user_center)));
			if (clickIndex == 0) {
				// 在线列表
				ivScan.setVisibility(View.VISIBLE);
				ivScan.setImageDrawable(getResources().getDrawable(
						R.drawable.qrcode_scan));
				toolbarMenu.setVisibility(View.GONE);
			} else if (clickIndex == 1) {// 如果为
				// 设备管理列表
				ivScan.setImageDrawable(getResources().getDrawable(
						R.drawable.manage_device_share2));
				ivScan.setVisibility(View.VISIBLE);
				toolbarMenu.setVisibility(View.GONE);
				...
				ivScan.setVisibility(View.GONE);
				toolbarMenu.setVisibility(View.VISIBLE);
				toolbarMenu.setText(R.string.update);
			} else {
				ivScan.setVisibility(View.GONE);// 扫描按钮消失
				toolbarMenu.setVisibility(View.GONE);// 右边菜单消失
			}
		}
	}
		...
		if (NetworkUtils.isNetworkConnected(this)) {
			...
			new Thread() {
				public void run() {
					...
					try {
						// HttpUtils.TOKIN = "";
						// String result ="";// HttpUserService.login(account,
						// pwd); //
						// "{\"code\":2,\"result\":{\"logincode\":1, \"result\":{\"id\":\"10001\"}}}";
						// Log.d("resultLogin", result);
						// if (result != null) {
						// JSONObject obj = new JSONObject(result);
						// String logincode = obj.getString("code");
						// if (logincode.equals("999999")) {
						// JSONObject res = obj.getJSONObject("result");
						// if (isRequesting) {
						// Editor e = ((BApplication)
						// getApplication()).getEditor();
						...
						// }
						// }
						String[] array = new String[] { "格力", "美的", "海尔" };
						StringBuffer sb = new StringBuffer();
