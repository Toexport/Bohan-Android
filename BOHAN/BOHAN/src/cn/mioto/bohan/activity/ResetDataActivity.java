...
public class ResetDataActivity extends BaseCheckDataMenuActivity {
	...
	private static final int TYPE_SUCCESS = 10,LOCATION_SUCCESS = 11,TYPE_FAIL = 12,LOCATION_FAIL = 13,BRAND_SUCCESS = 14,BRAND_FAIL = 15,MODIFY_SUCCESS = 16;
			...
			new Thread() {
				public void run() {
					...
					try {
						...
						String result = WebServiceClient.CallWebService("ModifyDeviceInfo",map);
				...
				public void run() {
					...
					try {
						...
						String result = WebServiceClient
								.CallWebService("GetLoadNameList",null);
						Log.d("resultLogin", result);
						if (result != null) {
							JSONObject obj = new JSONObject(result);
							int statusCode = obj.getInt("statusCode");
			...
			new Thread() {
				public void run() {
					...
					try {
						HttpUtils.TOKIN = "";
						String result = WebServiceClient
								.CallWebService("GetLoadBrandList",null);
						Log.d("resultLogin", result);
						if (result != null) {
							JSONObject obj = new JSONObject(result);
							int statusCode = obj.getInt("statusCode");
