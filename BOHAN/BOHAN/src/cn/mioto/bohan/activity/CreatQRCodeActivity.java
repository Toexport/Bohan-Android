...
public class CreatQRCodeActivity extends BaseActivity implements OnClickListener,MyQRCodeEncoder.MyEncoderDelegate {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ViewUtil.initToolbar(this, "分享设备", true, false, 0);
		Intent i = getIntent();
		...
		bindViews();
		sendIDAndUserNameToService(targetUserId,idsBuffer.toString());
		//		//生成一个全网唯一的序列号
	...
	private void sendIDAndUserNameToService(String user,String ids) {
		new Enterface("shareDevice.act").addParam("username", user).addParam("deviceids", ids).doRequest(new JsonClientHandler2() {

			@Override
			public void onInterfaceSuccess(String message, String contentJson) {
				LogUtilNIU.value("上传用户名和列表后服务器返回---->"+contentJson);
				createQRCode(contentJson);//把文字生成二维码
			}

			@Override
			public void onInterfaceFail(String json) {
				ToastUtils.shortToast(CreatQRCodeActivity.this, "用户不存在");
			}
